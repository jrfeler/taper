...
class Checkin extends React.Component {
  ...
  toggleContinueTaper() {
    this.setState({
      continueTaper: !this.state.continueTaper
    })
  }
  toggleFeedback(ind) {
    ...
    return (<div>
      <Box pad={'medium'}>
        ...
        <Section>
          ...
          <CheckBox label={'No'} checked={!this.state.continueTaper} onChange={() => this.toggleContinueTaper()}></CheckBox>
          <CheckBox label={'Yes'}  checked={this.state.continueTaper} onChange={() => this.toggleContinueTaper()}></CheckBox>
          {!this.state.continueTaper ? <Box>
              ...
              }
              {
                this.state.fbShow[1]
                  ? <Layer><Article>
                      ...
                      </Paragraph> <Button onClick={() => this.toggleFeedback(1)} label={'Close'}></Button></Article></Layer>

                  : null
              }
              {
                this.state.fbShow[2]
                  ? <Layer><Article><Paragraph>Lots of people feel that it is the medication that is keeping you functioning so if you lower the medication, your functioning will get worse. In fact, people usually function better off of opioids. You can check out our tips (links to resource page) on how to manage your pain in LIMIT. Also, it may help you to listen to some other veterans’ experiences.
                </Paragraph><Button onClick={() => this.toggleFeedback(2)} label={'Close'}></Button></Article></Layer>
                  : null
              }
              {
                this.state.fbShow[3]
                  ? <Layer><Article><Paragraph>Craving means you are preoccupied with the thought of getting more medication or being able to take medication right away. This is another problem Veterans can encounter when they reduce the dose of their medication too quickly. To avoid craving, just like to avoid withdrawal, the key is to reduce opioid medication gradually. The other important strategy to reduce or avoid craving is to have other activities and stay busy so that the thoughts of taking the next dose of medication are less strong.</Paragraph><Button onClick={() => this.toggleFeedback(3)} label={'Close'}></Button></Article></Layer>
                  : null
              }
              {
                this.state.fbShow[4]
                  ? <Layer><Article><Paragraph>A lot of Veterans feel like they’ve tried most everything already. A couple things to consider: You’re likely at a different place now – a different mindset, a different set of life circumstances – than you were when you tried some of those other things. Another try is probably worth a go! Also, there are new approaches coming on the scene all the time. Click here (links to page describing non-pharmacologic options at the VA – by site) to see some of the options available at the VA. Click on the XX to hear about how other Veterans dealt with this issue.</Paragraph><Button onClick={() => this.toggleFeedback(4)} label={'Close'}></Button></Article></Layer>
                  : null
              }
